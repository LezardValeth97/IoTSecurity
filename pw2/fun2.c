...
int main(int argc, char const *argv[])
{
	...
	{
		int keyfile = open("key.txt", O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR); // !exist -> don't read; already exist -> clean
		if (keyfile)
		{
			dup2(keyfile, 1);
		}
		...
		while (!isPrime || mpz_cmp(z_p, z_q) == 0 || !invertible)
		{
			...
			isPrime = mpz_probab_prime_p(z_q, 5); // Check prime q

			mpz_sub_ui(z_p_1, z_p, 1); // p = p-1
			mpz_sub_ui(z_q_1, z_q, 1); // q = q-1
		...
		if (keyfile)
		{
			//Ensure the result wrote to a file is written out on disk
			fflush(stdout);
			close(keyfile);
		}
		return 0;
